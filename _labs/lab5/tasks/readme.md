...
Логику мы настроили. Теперь осталось во `views.py` поменять приложения `app_store` поправить представление `cart_view`
...
Также необходимо поправить все вызовы функций `add_to_cart`, `remove_from_cart` во `views.py` приложения `app_store`, так как
